"""
Swiss Knife — Stochastic Strategies Harmlessness Benchmark (Knockout Bracket)
=============================================================================
...
────────────────────
  ...
  2. stochastic_knockout_dropout
       Same model + blade. Before each individual match in the knockout bracket,
  ...
  4. stochastic_knockout_subsample
...
──────────────
  ...
  python3 evaluation/benchmark_stochastic_strategies_knockout_bracket.py \\
      --num-prompts 15 \\
      --max-tokens 200 \\
      --blade harmlessness \\
      --alpha 0.5 \\
      --beta 0.1 \\
      --dtype bfloat16 \\
      --output-dir runs/stochastic_strategies_knockout_benchmark
"""
...
STRATEGIES = [
    ...
    ("stochastic_knockout_dropout",    "mc_dropout"),   # MC-Dropout per match
    ("stochastic_knockout_proj",       "random_proj"),  # Random projection per match
    ("stochastic_knockout_subsample",  "head_subsample"), # Head subsampling per match
]
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            "Benchmark 4 knockout-bracket strategies (1 baseline + 3 stochastic) "
    ...
    p.add_argument("--output-dir", type=str,
                   default="runs/stochastic_strategies_knockout_benchmark")
    p.add_argument("--verbose", action="store_true",
                   help="Log per-round tournament details")
    p.add_argument("--mock", action="store_true",
                   help="Offline mode: use lightweight toy modules instead of real models")
    p.add_argument("--start-from-strategy", type=str, default=None,
                   help="Skip all strategies before this one (use exact strategy name). "
                        "E.g. --start-from-strategy stochastic_knockout_proj")
    p.add_argument("--stop-after-strategy", type=str, default=None,
                   help="Stop after this strategy completes (inclusive). "
                        "E.g. --stop-after-strategy stochastic_knockout_dropout")
    return p.parse_args()
...
def main():
    ...
    # ── Optional: slice the strategy list ───────────────────────────────────
    names = [s for s, _ in STRATEGIES]
    active_strategies = list(STRATEGIES)

    if args.start_from_strategy:
        if args.start_from_strategy not in names:
            print(f"ERROR: --start-from-strategy '{args.start_from_strategy}' not found.")
            print(f"  Valid names: {names}")
            sys.exit(1)
        start_idx = names.index(args.start_from_strategy)
        active_strategies = active_strategies[start_idx:]
        print(f"  ⏩ Resuming from strategy #{start_idx + 1}: {args.start_from_strategy}")

    if args.stop_after_strategy:
        active_names = [s for s, _ in active_strategies]
        if args.stop_after_strategy not in active_names:
            print(f"ERROR: --stop-after-strategy '{args.stop_after_strategy}' not found in remaining strategies.")
            print(f"  Remaining: {active_names}")
            sys.exit(1)
        stop_idx = active_names.index(args.stop_after_strategy)
        active_strategies = active_strategies[:stop_idx + 1]  # inclusive
        print(f"  ⏹ Will stop after: {args.stop_after_strategy}")

    print("=" * 70)
    print("  Swiss Knife — Stochastic Strategies Harmlessness Benchmark (Knockout)")
    print("=" * 70)
    print(f"  Strategies    :")
    ...
    # ── Run each strategy ─────────────────────────────────────────────────────
    for strat_name, stoch_mode in active_strategies:
