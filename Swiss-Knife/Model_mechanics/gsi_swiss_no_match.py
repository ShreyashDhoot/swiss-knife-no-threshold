...
2. Tilted Reward Calculation: Compute tilted rewards for all `n` candidates.
3. Swiss Tournament (R rounds):
   ...
   - Match evaluation: Higher Tilted Reward wins the match.
   - Winner gets 1 point; loser gets 0 points.
4. Softmax Selection: Apply softmax (temperature β) over the final points 
   table to stochastically select a winning step.
5. Verification: Accept the selected step unconditionally, append to prefix, and repeat.
"""
...
def swiss_system_points_table(
    tilted_rewards: torch.Tensor,
    rounds: int = 0,
) -> Tuple[List[float], int, int]:
    """Run Swiss-system matches using calculated Tilted Rewards and return the cumulative points table."""
    n = tilted_rewards.shape[0]
    ...
    for rnd in range(rounds):
        ...
        # ── Execute matches using Tilted Reward comparisons ────────────
        for a, b in pairs:
            delta_tilted = tilted_rewards[a] - tilted_rewards[b]

            # The higher static tilted reward wins the match
            if delta_tilted > 0:
                winner, loser = a, b
            else:
                winner, loser = b, a
            ...
            logger.debug(
                "Swiss Round %d | c%d vs c%d → winner=c%d (Δtilted=%.4f)",
                rnd + 1, a, b, winner, delta_tilted.item(),
            )
...
class GSISwissGenerator:
    """GSI Strategy 3: Swiss-system → points table → softmax selection via Tilted Reward."""
    ...
    def __init__(
        ...
        logger.info(
            "GSISwissGenerator initialized: n=%d, β=%.3f, "
            "swiss_rounds=%d, threshold=%.3f",
            cfg.gsi_n, cfg.beta, cfg.swiss_rounds,
            cfg.gsi_threshold,
        )
    ...
    def generate(
        ...
        """Run GSI Strategy 3: Swiss-system → points → softmax (Without Fallback)."""
        ...
        while len(generated_tokens) < max_tokens:
            ...
            for i in range(n_actual):
                ...
                # Verifier logprob for all candidates to build the total tilted reward pool upfront
                target_lp = compute_logprob(self.verifier_model, prefix_ids_verifier, verifier_step_ids)
                target_logprobs_list.append(target_lp)
            ...
            target_logprobs = torch.tensor(target_logprobs_list, dtype=torch.float, device=self.verifier_device)

            blade_rewards = self.blade.score_reasoning_steps(prefix_ids_verifier.unsqueeze(0), verifier_step_ids_list)

            # ── Step 2: Compute Tilted Rewards for ALL candidates ────────────
            # R_tilted = r_blade + (1 / beta) * (log π_target - log π_draft)
            tilted_rewards = blade_rewards + (1.0 / beta) * (target_logprobs - draft_logprobs)

            # ── Step 3: Swiss-system tournament via Tilted Rewards ───────────
            ...
            # ── Step 5: Assign Winner (No Threshold / Fallback) ─────────────
